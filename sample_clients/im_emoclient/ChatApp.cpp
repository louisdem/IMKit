...
#include <Alert.h>
#include "PopUpMenu.h" //local implementation
	...
	fIsQuiting(false)
{
	...
	entry.GetPath( &path );
	path.GetParent( &path );
	path.Append( "Language/Dictionaries/im_emoclient" );
	BString path_string;
	
	if( path.InitCheck() != B_OK )
		path_string.SetTo( "Language/Dictionaries/im_emoclient" );
	else
...
//	userColor.AddInt32("type", B_RGB_COLOR_TYPE);
	...
	if ( !settings.FindString("app_sig") )
		settings.AddString("app_sig", SIGN);
	...
	if (settings.FindString("smiley_config", &smiley_config) != B_OK ) {
		smiley_config.SetTo( "/boot/home/config/settings/im_kit/smileys/settings.xml");
	...
	emoticor=new Emoticor;
	emoticor->LoadConfig(smiley_config.String());
	MenuBuilder menub;
	popup=menub.CreateMenuP(&emoticor->config->menu,978);  //FIX the BMessage id!
}
			...
			if ( reply.FindBool("isShuttingDown", &isShuttingDown) == B_OK )
			{
				if ( !isShuttingDown )
				{
					BAlert * alert = new BAlert(
						_T("Really quit?"), 
						_T("Do you really want to quit im_emoclient and stop getting messages?"),
						_T("Yes"),
						_T("No")
					);
...
ChatApp::RefsReceived( BMessage * msg )
{
	...
	for ( int i=0; msg->FindRef("refs", i, &ref ) == B_OK; i++ ) {
		...
		if (type != NULL && strcmp(type, "application/x-person") == 0) {
			...
			LOG("im_emoclient", liLow, "Got a ref that wasn't a People file");
		};
		free(type);
	};
	...
	switch ( msg->what )
	{
		...
		case IM::MESSAGE:
		{
			...
			{ // open new window on message received or user request
				LOG("im_emoclient", liMedium, "Creating new window to handle message");
...
//						Was opened by the user, so make focus
				...
				{
					LOG("im_emoclient", liHigh, "This is a fatal error that should never occur. Lock fail on new win.");
				}
			} else
			{
				if (_msgr.LockTarget() )
				...
				{
					LOG("im_emoclient", liHigh, "This is a fatal error that should never occur. Lock fail on old win.");
				}
			}
		}	break;
		default:
