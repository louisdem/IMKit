...
class PrivatePlaylist
{
	public:
		...
		void		GetRefList(BMessage &mes) const;
...
PLAYLIST_SECRET_INNER_WORKINGS
		...
		status_t pFetchQuery(BString predicate, BVolume *volume, int32 index);
		void pAddQueryToList(const entry_ref *ref, int32 index);
