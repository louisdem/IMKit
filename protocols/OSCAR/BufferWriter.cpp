...
uchar *BufferWriter::Buffer(void) {
	return fBuffer;
};

int32 BufferWriter::Length(void) {
	return fLength;
};

int32 BufferWriter::Offset(void) {
	return fOffset;
};

void BufferWriter::OffsetTo(int32 to) {
	fOffset = to;
};

void BufferWriter::OffsetBy(int32 by) {
	fOffset += by;
};

//#pragma mark Reader Methods
                                     
void BufferWriter::WriteInt8(int8 value) {
	Allocate(sizeof(int8));
	memcpy(fBuffer + fOffset, (void *)&value, sizeof(int8));
	fOffset += sizeof(int8);
};

void BufferWriter::WriteInt16(int16 value) {
...
void BufferWriter::WriteInt32(int32 value) {
...
void BufferWriter::WriteInt64(int64 value) {
...
void BufferWriter::WriteData(const uchar *data, int16 length) {
	Allocate(length);
...
void BufferWriter::WriteString(const char *data, int16 length) {
	Allocate(length);
...
void BufferWriter::Allocate(int16 length) {
	if ((fOffset + length) >= fAllocated) {
		fBuffer = (uchar *)realloc(fBuffer, sizeof(uchar) *
