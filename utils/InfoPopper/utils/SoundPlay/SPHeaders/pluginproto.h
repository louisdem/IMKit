...
typedef void*		op_instantiate(void **data, const char *name, const char *header, uint32 size, plugin_info *pluginfo, SoundPlayBufferedReader *reader);
...
typedef struct plugin_descriptor
{
	...
	uint32				flags;					// PLUGIN_IS_DECODER, PLUGIN_IS_FILTER, PLUGIN_IS_VISUAL, PLUGIN_IS_INTERFACE
...
typedef	status_t	op_decoder_open(void*,const char *name, const char *header, uint32 size, BMessage *config, SoundPlayBufferedReader *reader);
...
class SoundPlayReader
{
	public:
		virtual				~SoundPlayReader();
		virtual status_t	InitCheck() = 0;
		virtual bool		IsRandomlyAccessible() = 0;
		...
		virtual const char	*MetaData();	// return NULL if no new metadata since last time called
};

class SoundPlayBufferedReader: public SoundPlayReader
{
	public:
		virtual float		BufferPercentage() = 0;
};
...
public:
	...
	// plugins can set a 'metadata' string.
	virtual status_t	SetMetaData(const char *meta, void *plugindata);
...
enum plugin_type {
	...
	PLUGIN_NEEDS_READER=0x20000000			// for decoders only: pass in a SoundPlayReader (the plugin owns it after this)
};
